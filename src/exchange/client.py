...
class ExchangeClient(IExchange):
    ...
    async def _make_request(self, endpoint: str, params: Dict, method: str = "GET") -> Dict:
        ...
        async with ClientSession() as session:
            if method == "GET":
                ...
                async with session.post(f"{self.base_url}{endpoint}", params=params, headers=headers) as resp:
                    if resp.status != 200:
                        logger.error("API request failed: %s, %s", endpoint, await resp.text())
                        raise Exception(f"API error: {resp.status}")
                    data = await resp.json()
